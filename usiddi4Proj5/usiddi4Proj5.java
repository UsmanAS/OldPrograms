 ...
 * Date: 3/10/16
 * 
 * @Author - Usman Siddiqui, UIC, CS Department
 * 
 */
...
public static void processExpression (Token inputToken, TokenReader tr)
{
 ...
 /* Declare both stack head pointers here      */
  ...
  linked.createOpStack();
  
  //Node OpStack = new Node();
  //Node ValStack = new Node();
 ...
 while (inputToken.equalsType(TokenType.EOLN) == false)
   {
    ...
    if (inputToken.equalsType(TokenType.OPERATOR))
      ...
      if(inputToken.getOperator() == '(')
               linked.opStack.push(inputToken.getOperator());
      if(inputToken.getOperator() == '-' || inputToken.getOperator() == '+'){
         while(linked.opStack.next.isEmpty() == 0 && (inputToken.getOperator() == '+' || inputToken.getOperator() == '-' || inputToken.getOperator() == '/' || inputToken.getOperator() == '*'))
             popAndEval(linked);
         linked.opStack.push(inputToken.getOperator());
      }
      if(inputToken.getOperator() == '*' || inputToken.getOperator() == '/'){
        while(linked.opStack.head.next.isEmpty() == 0 && (inputToken.getOperator() == '/' || inputToken.getOperator() == '*'))
             popAndEval(linked);
        linked.opStack.push(inputToken.getOperator());
      }
      if(inputToken.getOperator() == ')'){
        while(linked.opStack.next.isEmpty() == 0 && inputToken.getOperator() != '(')
             popAndEval(linked);
        if(linked.opStack.isEmpty() == 1)
    ...
    /* The expression contain a VALUE */
      ...
      {
       /* make this a debugMode statement */
      linked.valStack.push(inputToken.getValue());
      if(Bug.DEBUG == 1)
 ...
 while(linked.opStack.next.isEmpty() == 0)
   popAndEval(linked);

 /* The expression has reached its end */

 // add code to perform this operation here
 System.out.print("Final number: " + linked.valStack.topVal());
 linked.valStack.pop();
 System.out.print ("\n");
}

  public static void popAndEval(LinkedList linked){
  char op = linked.opStack.topOp();
  if( op != '+' && op != '-' && op != '*' && op != '/'){
    System.out.println("Error.");
    return;
  }
  ...
  linked.opStack.pop ();
  v2 = linked.valStack.topVal();
  linked.valStack.pop ();
  v1 = linked.valStack.topVal();
  linked.valStack.pop ();
  v3 = eval ( v1, op, v2 );
  linked.valStack.push (v3);
}
