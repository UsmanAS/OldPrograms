...
function createCone( nSectors, gl, color ) {
	...
	for( var i = 0; i < nSectors + 2; i++ ) {
	    if( validColor )
	        colors.push(vec3(color[0],color[1],color[2]));// Push a valid color here, as a vec3
	    else
	...
	for( i = 0; i < nSectors + 1; i++ ) { // Duplicate ( 1, 0, 0 ) to close loop.
		var theta = i * dTheta;
		points.push(vec3(Math.cos(theta), 0, Math.sin(thetha)));// push a vertex here, using Math.cos( theta ) for X and Math.sin( theta ) for Y
	}	
